module "management_services_virtual_machines_prod" {
...
module "management_services_network_peering_prod" {
  ...
  service_name               = var.service_name
  service_location           = each.value
  resource_network_peer      = module.management_services_virtual_machines_prod[each.value]
  resource_network_peer_role = var.resource_network_peer_role
}

module "management_services_traffic_manager_prod" {
  depends_on                                  = [module.management_services_virtual_machines_prod]
  ...
  resource_name                               = local.resource_name
  resource_traffic_manager_endpoints          = module.management_services_virtual_machines_prod
...
module "management_services_recovery_services_prod" {
  ...
  resource_recovery_services_virtual_machines = module.management_services_virtual_machines_prod[each.value]
}